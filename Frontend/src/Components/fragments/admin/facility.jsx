...
const Facility = () => {
  ...
  const handleAddFacilitySubmit = async (e) => {
    ...
    try {
      await axios.post(
        "https://ceria-music-production-4534.up.railway.app/api/facility",
        addFacilityForm,
        {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("token")}`,
  ...
  const handleEditFacilitySubmit = async (e) => {
    ...
    try {
      await axios.put(
        `https://ceria-music-production-4534.up.railway.app/api/facility/${editFacilityForm.id}`,
  ...
  const handleDeleteFacility = (facilityId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(
            `https://ceria-music-production-4534.up.railway.app/api/facility/${facilityId}`,
            {
              headers: {
                Authorization: `Bearer ${sessionStorage.getItem("token")}`,
              },
